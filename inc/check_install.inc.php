...
if(empty($_POST['manage_pwd']) || mb_strlen($_POST['manage_pwd'])<5){

	exit('后台管理员密码不得少于5位！<a href="install.php">点击返回</a>');
}
if(empty($_POST['manage_pw_confirm']) || $_POST['manage_pwd']!=$_POST['manage_pw_confirm']){
	exit('两次密码输入不一致！<a href="install.php">点击返回</a>');
}
$link=@mysqli_connect($_POST['db_host'],$_POST['db_user'],$_POST['db_pwd'],'',$_POST['port']);
